...
stmtParserTests = testParser "statemet parser tests" $ do
  describe "connect" $ do
    it "standard" $
      parseStmt "somenode <= UInt(15)"
      `shouldParse`
      Syntax.Connect
        (Syntax.Ref "somenode")
        (Syntax.Lit (Syntax.UInt 15))
  ...
  it "node" $
    parseStmt "node a = UInt(15)"
    `shouldParse`
    Syntax.Node "a" (Syntax.Lit (Syntax.UInt 15))

  describe "printfs" $ do
    it "without optional args" $
      parseStmt "printf(UInt(1), UInt(1), \"Test\")"
      `shouldParse`
      Syntax.Print (Syntax.Lit (Syntax.UInt 1))
                   (Syntax.Lit (Syntax.UInt 1))
                   "Test"
                   []

    it "with 1 optional arg" $
      parseStmt "printf(UInt(1), UInt(1), \"Test %d\", UInt(2))"
      `shouldParse`
      Syntax.Print (Syntax.Lit (Syntax.UInt 1))
                   (Syntax.Lit (Syntax.UInt 1))
                   "Test %d"
                   [Syntax.Lit (Syntax.UInt 2)]

    it "with multiple optional arg" $
      parseStmt "printf(UInt(1), UInt(1), \"Test %d %d %d\", UInt(2), UInt(15), SInt(-3))"
      `shouldParse`
      Syntax.Print (Syntax.Lit (Syntax.UInt 1))
                   (Syntax.Lit (Syntax.UInt 1))
                   "Test %d %d %d"
                   [ Syntax.Lit (Syntax.UInt 2)
                   , Syntax.Lit (Syntax.UInt 15)
                   , Syntax.Lit (Syntax.SInt (-3))
                   ]

    it "stop" $
      parseStmt "stop(UInt(2), UInt(13), -4)"
      `shouldParse`
      Syntax.Stop (Syntax.Lit (Syntax.UInt 2))
                  (Syntax.Lit (Syntax.UInt 13))
                  (-4)
    ...
    describe "wires" $ do
      it "unsigned" $
        ...
        `shouldParse`
        Syntax.Wire "drut" (Types.Unsigned 14)

      it "signed" $
        parseStmt "wire druciak : SInt<2>"
        `shouldParse`
        Syntax.Wire "druciak" (Types.Signed 2)

      it "clock" $
        parseStmt "wire clk: Clock"
        `shouldParse`
        Syntax.Wire "clk" Types.Clock
